...
//https://stepik.org/lesson/498100/step/6
final class CommentUtils {
    ...
    public static void handleComments(List<Comment> comments, Date thresholdDate, int maxTextLength) {
        ...
        // My first solution, using a for loop
        /* for (int i=0; i < comments.size(); i++) {
            if (comments.get(i).getText().length() >= maxTextLength) {
               String commentWithMaxSize = comments.get(i).getText().substring(0, maxTextLength);
               comments.set(i, new Comment(comments.get(i).getCreated(), commentWithMaxSize));
            }
        } */
    ...
    public static void printComments(List<Comment> comments) {
        comments.forEach(comment ->
                System.out.println("[" + TEXT_FORMATTER.format(comment.getCreated()) + "] " + comment.getText()));// write your code here
    }
}
